...
(function (root) {
  ...
  var Sequence = (function() {
    ...
    function Sequence (methods) {
      var newInstance, name;
      
      newInstance = this instanceof Sequence
        ? this
        : new Sequence()
      
      for (name in methods) {
        if (!__hasProp.call(methods, name)) continue;
        newInstance[name] = methods[name];
      }
      newInstance.of || (newInstance.of = function(value) {
        return value;
      });
      newInstance.map || (newInstance.map = function(fn) {
        return fn;
      });
      newInstance.chain || (newInstance.chain = function(mValue, fn) {
        return newInstance.map(fn)(mValue);
      });
      for (name in newInstance) {
        if (!__hasProp.call(newInstance, name)) continue;
        newInstance[name] = newInstance[name].bind(newInstance);
      }
    }
    ...
    var naiveSequence = flip(compose);

    Sequence.sequence = function () {
      var fns, supervisor, arity, lastFn;
      if (arguments.length === 0) {
        return function (value) { return value; } // I combinator
      }
      else if (arguments[0] instanceof Sequence) {
        supervisor = arguments[0];
        fns = 2 <= arguments.length ? __slice.call(arguments, 1) : [];
        arity = fns.length > 0
          ? fns[0].length
          : 0;
        return variadic(arity, function (args) {
          return fns.reduce(supervisor.chain, supervisor.of.apply(supervisor, args));
        });
      }
      else if (isArray(arguments[0])) {
        fns = 2 <= arguments.length ? __slice.call(arguments, 1) : [];
        fns = arguments[0].reduce(function (functionsToTransform, decorator) {
          var result = map(functionsToTransform, decorator);
          return result;
        }, fns);
          console.log('fns', fns);
        return naiveSequence.apply(this, fns);
      }
      else {
        return naiveSequence.apply(this, arguments);
      }
    };
